  ...
  public class TestNotesResource extends TestCase
  {
    ...
    public function testList():void{
      ...
      var responder:IResponder = asyncResponder( 
            new TestResponder( assertTestList, handleFault ),
                               1000, null, handleTimeOut );
      call.addResponder(responder);  
    }
    public function assertTestList(result:ResultEvent,
                                   passThroughData:Object):void {
      assertEquals(2, result.result.note.length());
      assertEquals("MyString", result.result.note[0].title);
    }
    protected function handleFault(fault:FaultEvent,
                                   passThroughData:Object):void {
       fail("Failed to invoke notesResource."+fault.fault.faultString);
    }        
